...
class Socket
{
  public:
    ...
    int getSockFD();
    inline void setSockFD(int new_fd){sockFD=new_fd;}
  ...
  public:
    ...
    :Exception(exception)
    {
      switch(exception)
      {
        ...
        case E_SOCKET_FAIL:setError("Cannot open socket");break;
        case E_CLOSE_FAIL:setError("Cannot close socket");break;
        case E_BIND_FAIL:setError("Cannot bind port:"+param+"to socket");break;
        case E_SEND_FAIL:setError("An error occured while data was being sent:"+param);break;
        case E_RECV_FAIL:setError("An error occured while data was being recieved");break;
      }
    }
};
